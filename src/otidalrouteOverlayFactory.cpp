...
bool otidalrouteOverlayFactory::RenderGLotidalrouteOverlay( wxGLContext *pcontext, PlugIn_ViewPort *vp )
{
...
bool otidalrouteOverlayFactory::RenderotidalrouteOverlay( wxDC &dc, PlugIn_ViewPort *vp )
{
...
bool otidalrouteOverlayFactory::DoRenderotidalrouteOverlay( PlugIn_ViewPort *vp )
{
...
void otidalrouteOverlayFactory::DrawMessageWindow( wxString msg, int x, int y , wxFont *mfont)
{
    if(msg.empty())
        return;
...
void otidalrouteOverlayFactory::drawCurrentArrow(int x, int y, double rot_angle, double scale, double rate )
{   	
	double m_rate = rate;
	wxPoint p[9];
...
void otidalrouteOverlayFactory::DrawOLBitmap( const wxBitmap &bitmap, wxCoord x, wxCoord y, bool usemask )
{
    wxBitmap bmp;
    if( x < 0 || y < 0 ) {
        int dx = ( x < 0 ? -x : 0 );
...
void otidalrouteOverlayFactory::DrawGLLabels(otidalrouteOverlayFactory *pof, wxDC *dc,
                                PlugIn_ViewPort *vp,
...
void otidalrouteOverlayFactory::drawGLPolygons(otidalrouteOverlayFactory *pof, wxDC *dc,
                                PlugIn_ViewPort *vp,
                                wxImage &imageLabel, double myLat, double myLon, int offset )
{
...
void otidalrouteOverlayFactory::DrawAllCurrentsInViewPort(PlugIn_ViewPort *BBox, bool bRebuildSelList,
