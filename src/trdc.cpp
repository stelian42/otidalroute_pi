 ...
 *   along with this program; if not, trite to the                         *
...
void trDC::SetBackground( const wxBrush &brush )
{
    if( dc )
...
void trDC::SetPen( const wxPen &pen )
{
    if( dc ) {
...
void trDC::SetBrush( const wxBrush &brush )
{
...
void trDC::SetTextForeground( const wxColour &colour )
{
...
void trDC::SetFont( const wxFont& font )
{
...
const wxPen& trDC::GetPen() const
{
    if( dc ) return dc->GetPen();
    return m_pen;
}

const wxBrush& trDC::GetBrush() const
{
    if( dc ) return dc->GetBrush();
    return m_brush;
}

const wxFont& trDC::GetFont() const
{
    if( dc ) return dc->GetFont();
    return m_font;
}

void trDC::GetSize( wxCoord *width, wxCoord *height ) const
{
    if( dc )
...
bool trDC::s_pushed_blend = false;
void trDC::SetGLStipple(int style, bool needs_blend)
{
#ifdef ocpnUSE_GL
...
void trDC::DrawLine( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2, bool b_hiqual )
{
    if( dc )
...
void trDC::DrawLines( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset, bool b_hiqual )
{
    if( dc )
...
void trDC::StrokeLine( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2 )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
...
void trDC::StrokeLines( int n, wxPoint *points) {
...
void trDC::DrawRectangle( wxCoord x, wxCoord y, wxCoord w, wxCoord h )
{
    if( dc )
...
static void dratrrhelper( wxCoord x, wxCoord y, wxCoord r, float st, float et )
{
#ifdef ocpnUSE_GL
    const int slices = 10;
...
void trDC::DrawRoundedRectangle( wxCoord x, wxCoord y, wxCoord w, wxCoord h, wxCoord r )
{
    if( dc )
        ...
        if( ConfigureBrush() ) {
            ...
            glVertex2i( x1, y2 );
            dratrrhelper( x1, y2, r, (float)M_PI / 2., (float)M_PI );
            glEnd();

            glBegin( GL_TRIANGLE_FAN );
            glVertex2i( x2, y2 );
            dratrrhelper( x2, y2, r, 0, (float)M_PI / 2. );
            glEnd();

            glBegin( GL_TRIANGLE_FAN );
            glVertex2i( x2, y1 );
            dratrrhelper( x2, y1, r, (float)-M_PI / 2., 0 );
            glEnd();

            glBegin( GL_TRIANGLE_FAN );
            glVertex2i( x1, y1 );
            dratrrhelper( x1, y1, r, (float)-M_PI, (float)-M_PI / 2. );
            glEnd();
        ...
        if( ConfigurePen() ) {
            glBegin( GL_LINE_LOOP );
            dratrrhelper( x1, y2, r, (float)-M_PI, (float)-M_PI / 2. );
            dratrrhelper( x2, y2, r, (float)-M_PI / 2., 0 );
            dratrrhelper( x2, y1, r, 0, (float)M_PI / 2. );
            dratrrhelper( x1, y1, r, (float)M_PI / 2., (float)M_PI );
            glEnd();
        }
...
void trDC::DrawCircle( wxCoord x, wxCoord y, wxCoord radius )
{
    DrawEllipse( x - radius, y - radius, 2 * radius, 2 * radius );
}

void trDC::StrokeCircle( wxCoord x, wxCoord y, wxCoord radius )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
...
void trDC::DrawEllipse( wxCoord x, wxCoord y, wxCoord width, wxCoord height )
{
    if( dc )
...
void trDC::DrawPolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset )
{
    if( dc )
...
void trDC::StrokePolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
        wxGraphicsPath gpath = pgc->CreatePath();
...
void trDC::DrawBitmap( const wxBitmap &bitmap, wxCoord x, wxCoord y, bool usemask )
{
...
void trDC::GetTextExtent( const wxString &string, wxCoord *w, wxCoord *h, wxCoord *descent,
        wxCoord *externalLeading, wxFont *font ) const
{
    ...
    if( dc ) dc->GetTextExtent( string, w, h, descent, externalLeading, font );
     ...
     //  Sometimes GetTextExtent returns really trong, uninitialized results.
     //  Dunno why....
     if( w && (*w > 500) ) *w = 500;
     if( h && (*h > 500) ) *h = 500;
}

void trDC::ResetBoundingBox()
{
    if( dc ) dc->ResetBoundingBox();
}

void trDC::CalcBoundingBox( wxCoord x, wxCoord y )
{
    if( dc ) dc->CalcBoundingBox( x, y );
}

bool trDC::ConfigurePen()
{
...
void trDC::GLDrawBlendData( wxCoord x, wxCoord y, wxCoord w, wxCoord h, int format,
