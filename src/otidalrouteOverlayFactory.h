 ...
 *
 * Project:  OpenCPN
 * Purpose:  otidalroute Plugin Freinds
...
//----------------------------------------------------------------------------------------------------------
//    otidalroute Overlay Specification
//----------------------------------------------------------------------------------------------------------

class otidalrouteOverlay {
public:
    otidalrouteOverlay( void )
    {
        m_iTexture = 0;
...
public:
    otidalrouteOverlayFactory( otidalrouteUIDialog &dlg );
    ~otidalrouteOverlayFactory();
	...
	bool RenderGLotidalrouteOverlay( wxGLContext *pcontext, PlugIn_ViewPort *vp );
    bool RenderotidalrouteOverlay( wxDC &dc, PlugIn_ViewPort *vp );
	...
	void DrawAllTidalCurrentsInViewPort(double dlat, double dlon, double ddir, double dfor, PlugIn_ViewPort *myVP);
	void DrawAllGLTidalCurrentsInViewPort(double dlat, double dlon,
		double ddir, double dfor, wxGLContext *pcontext, PlugIn_ViewPort *myVP);
	...
	void DrawGLLabels(otidalrouteOverlayFactory *pof, wxDC *dc,
                               PlugIn_ViewPort *vp,
                               wxImage &imageLabel, double myLat, double myLon, int offset);

	void drawGLPolygons(otidalrouteOverlayFactory *pof, wxDC *dc,
                                PlugIn_ViewPort *vp, 
                                wxImage &imageLabel, double myLat, double myLon, int offset );
...
private:
    ...
    bool DoRenderotidalrouteOverlay( PlugIn_ViewPort *vp );
