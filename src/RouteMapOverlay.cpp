...
static void SetColor(trdc &dc, wxColour c, bool penifgl = false)
{
    if(!dc.GetDC()) {
...
static void SetWidth(trdc &dc, int w, bool penifgl = false)
{
    if(!dc.GetDC()) {
        glLineWidth(w);
        if(!penifgl)
...
void RouteMapOverlay::Render(wxDateTime time, SettingsDialog &settingsdialog,
                             trdc &dc, PlugIn_ViewPort &vp, bool justendroute)
{
    if(!justendroute) {
        RouteMapConfiguration configuration = GetConfiguration();
...
void RouteMapOverlay::RenderWindBarbs(trdc &dc, PlugIn_ViewPort &vp)
{
...
void RouteMapOverlay::RenderCurrent(trdc &dc, PlugIn_ViewPort &vp)
{
    if(origin.size() < 2) // no map to work with
        return;
